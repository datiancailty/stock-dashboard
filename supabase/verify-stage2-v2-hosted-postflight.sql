...
select check_group, object_name, observed, expected, pass
from checks
order by check_group, object_name;

-- Role privilege matrix. This is catalog privilege inspection, not an RPC call.
-- It intentionally checks only whether EXECUTE/table privileges exist.
with expected_function_privileges(function_signature, role_name, should_execute) as (
  values
...
-- No direct table privilege for the service_role on any private control table.
...
order by e.table_name, p.privilege_name;

-- Authenticated receives only the audit projection's table SELECT permission among
-- the three v2-only tables; RLS still controls which rows an admin can see.
select
