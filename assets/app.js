...
function renderHoldings(){
  ...
  const historyState=privateHistoryStatusHtml('holdings','Part 1 原自选清单');
  const projectionState=noSession?historyState:noProjection?'<p class="muted">当前账户没有可读取的 primary 持仓投影，需由项目管理员单独授予 scope membership。</p>':rows||'<p class="muted">当前私有投影未包含持仓行；这不是公开数据推断。</p>';
  const legacyState=authSession&&personalLoadedParts.has('holdings')?`<section class="legacy-watchlist"><div class="legacy-watchlist-head"><div><b>原 Part 1 自选清单</b><small>已迁入当前账号的 Supabase 私有空间；不等于模拟盘实际持仓</small></div><span>${trackedStocks.length}只</span></div><div class="legacy-watchlist-grid">${trackedStocks.map(item=>`<span><b>${escapeHtml(item.name||item.code)}</b><small>${escapeHtml(item.code)}</small></span>`).join('')||'<p class="muted">迁移清单为空</p>'}</div></section>`:authSession?historyState:'';
