...
VARIANT_DEFS=[
 ('周下+日下+月中','下部','下部','中部',True),('周下+日下+月下','下部','下部','下部',False),
 ('周下+日中+月中','下部','中部','中部',False),('周下+日中+月下','下部','中部','下部',False),
 ('周中+日下+月中','中部','下部','中部',False),('周中+日下+月下','中部','下部','下部',False),
 ('周中+日中+月中','中部','中部','中部',False),('周中+日中+月下','中部','中部','下部',False),
]
...
def cluster(samples):
 episodes=[]
 for item in samples:
  if not episodes or (datetime.fromisoformat(item['date']).date()-datetime.fromisoformat(episodes[-1][-1]['date']).date()).days>4: episodes.append([item])
  else: episodes[-1].append(item)
 return episodes
...
def variant_summary(rows,definition):
 signal,week_zone,day_zone,month_zone,preferred=definition; samples=[]
 for i,r in enumerate(rows):
  if r['date']<datetime(2024,1,1).date(): continue
  iso=r['date'].isocalendar()[:2]; week=[x for x in rows[:i+1] if x['date'].isocalendar()[:2]==iso]; month=[x for x in rows[:i+1] if (x['date'].year,x['date'].month)==(r['date'].year,r['date'].month)]
  dz,dp=position(r['close'],[r]); wz,wp=position(r['close'],week); mz,mp=position(r['close'],month)
  if (dz,wz,mz)!=(day_zone,week_zone,month_zone): continue
  forward={n:(rows[i+n]['close']/r['close']-1) if i+n<len(rows) else None for n in (5,10,20,30,60)}; path=rows[i+1:min(len(rows),i+31)]
  samples.append({'date':str(r['date']),'forward':forward,'max30':max((x['high']/r['close']-1 for x in path),default=None),'min30':min((x['low']/r['close']-1 for x in path),default=None)})
 episodes=cluster(samples); first=[e[0] for e in episodes]
 return {'signal':signal,'preferred':preferred,'signalDays':sample_stats(samples),'episodes':len(episodes),'episodeFirst':sample_stats(first)}
...
def analyze(code):
 ...
 def stats(samples):
  ...
  z['max30MedianPct']=round(statistics.median(ups)*100,2) if ups else None; z['drawdown30MedianPct']=round(statistics.median(dds)*100,2) if dds else None
  return z
 return {'code':code,'name':NAMES[code],'sourceRows':len(rows),'sourceRange':[str(rows[0]['date']),str(rows[-1]['date'])],'allSignalDays':stats(signals),'episodeFirstSignal':stats([e[0] for e in episodes]),'episodes':len(episodes),'episodeDates':[[e[0]['date'],e[-1]['date'],len(e)] for e in episodes],'variantStudy':{'preferredSignal':PREFERRED_SIGNAL,'testedSignals':[item[0] for item in VARIANT_DEFS],'variants':[variant_summary(rows,item) for item in VARIANT_DEFS]}}

result={'schemaVersion':2,'generatedAt':datetime.now().astimezone().isoformat(timespec='seconds'),'dataSource':'东方财富妙想金融数据（前复权日线）','evaluationWindow':'2024-01-01至数据最新交易日','indicatorDefinition':{'position':'当前收盘价在高低区间中的百分位；下部<33.33%，中部为33.33%至<66.67%，上部≥66.67%','day':'当日最高/最低','week':'ISO周内截至当日最高/最低','month':'自然月内截至当日最高/最低','signal':PREFERRED_SIGNAL,'testedSignalSpace':'周中/下 + 日中/下 + 月中/下，共8种组合','antiLookahead':'周/月只使用截至信号日已经出现的数据；未来收益使用前复权收盘价；不计分红、费用和滑点。'},'stocks':[analyze(c) for c in SOURCES]}
