...
每次有意义变更同步当前MD，包括成功、失败/未知与刻意暂停；以内容覆盖到的最后已确认事件为准，不以文件mtime推定完整。每个新tag必须保存同版本MD：公共仓库/Release仅脱敏说明，完整交接在私有维护仓库的根MD及handoffs/<tag>/冻结副本。私有校验器检查版本、正文hash、证据截止、分类与源码清单；失败不发布。发布后独立receipt绑定commit/tag/附件hash，避免正文自引用。后续新版本不覆盖旧tag或旧冻结MD。该流程不启用任何旧业务工作流。
