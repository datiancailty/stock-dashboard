...
- 每次用最近 35 个自然日滚动重扫 Part 4 官方公告；私有行情独立执行，公告核对失败或超时不再阻止行情刷新；未来分红网格只在本轮公告阶段成功后执行，否则保留上一份数据；
- 任一来源、分页、覆盖或私有 RPC 写入失败时不写入当天全流程成功状态，下一次周期再完整重试；汇总中的 `stages`、`quoteSucceeded` 分别报告局部结果，不能把行情成功当作公告/未来分红也成功；
- launchd 不继承交互终端的 Key。需要妙想的子任务在环境缺少 `MX_APIKEY` 时，从本机 `~/.hermes/workspace/stock-dashboard-private-runtime/credentials/mx-apikey` 读取；该文件必须为当前用户所有的普通文件、权限 `0600`，父目录权限 `0700`，拒绝符号链接和空内容。凭据不写入仓库、plist 或日志；独立行情不依赖该文件；
