-- Personal strategy research-match confidence contract v3 — forward-only migration.
...
begin
  ...
  if status_text is null or status_text not in ('ok', 'failed', 'unknown') then
  ...
  if status_text = 'ok' then
    ...
    if p_analysis->>'schemaVersion' is distinct from '3'
    ...
    if p_analysis->>'confidenceScale' is distinct from 'research_match_percent_0_to_100'
       or p_analysis->>'confidenceMeaning' is distinct from '研究匹配度，不是涨跌概率、收益概率或自动下单依据' then
      raise exception 'worker_confidence_contract_invalid';
    end if;
    ...
    if confidence_value is null
       or confidence_value < 0
       or confidence_value > 100
       or trunc(confidence_value) <> confidence_value then
      raise exception 'worker_brief_confidence_invalid';
    end if;
