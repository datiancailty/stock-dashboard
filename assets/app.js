...
function render(){renderTodayBoard();renderHoldings();renderPositions();renderGrid();renderCalendar();renderNews();renderStrategy();const d=market.updatedAt?new Date(market.updatedAt):null;$('#updateText').textContent=d?`私有行情快照采集 · ${d.toLocaleString('zh-CN',{hour12:false})}`:'私有快照 · 登录后读取';}
...
function renderGrid(){
  ...
  const validItems=items.filter(item=>Number(item.price)>0||forwardGridDividend(item)>0);
  ...
  $('#gridBody').innerHTML=items.length?items.map(s=>{const formalParts=[];if(Number(s.annualDividend)>0)formalParts.push(`年报 ${Number(s.annualDividend).toFixed(4)}`);if(Number(s.interimDividend)>0)formalParts.push(`中报 ${Number(s.interimDividend).toFixed(4)}`);const gridDividend=forwardGridDividend(s),isFuture=Number(s.futureDividend)>0,currentYield=Number(s.price)>0&&gridDividend>0?gridDividend/Number(s.price)*100:null;return `<tr><td><b>${escapeHtml(s.name||s.code)}</b><div class="stock-code">${escapeHtml(s.code)}</div></td><td><span>${escapeHtml(isFuture?`待实施 ${gridDividend.toFixed(4)}`:(formalParts.join(' + ')||'待公告分红'))}</span><small class="source-pill">${escapeHtml(forwardGridSource(s))}</small></td><td><b>${gridDividend>0?gridDividend.toFixed(4):'—'}</b><div class="stock-code">元/股 · 网格口径</div></td><td><b>${Number(s.price)>0?money(s.price):'—'}</b><div class="current-yield">${currentYield===null?'—':`${currentYield.toFixed(3)}%`}</div></td>${[5,5.5,6,6.5,7].map(rate=>gridTargetCell(s,rate)).join('')}</tr>`;}).join(''):'<tr><td colspan="9" class="muted">当前账号没有私有自选股票。</td></tr>';
}
function normalizedPart2Groups(){const groups=Array.isArray(part2Config.groups)?part2Config.groups:[],assigned=new Set(groups.flatMap(g=>g.codes||[])),other=groups.find(g=>g.name==='其他');for(const h of trackedStocks)if(!assigned.has(h.code)){if(!other){groups.push({name:'其他',hydropower:false,codes:[h.code]});assigned.add(h.code);}else if(!other.codes.includes(h.code)){other.codes.push(h.code);assigned.add(h.code);}}return groups;}
...
function bollYield(s){const dividend=forwardGridDividend(s);return s.price&&dividend?dividend/s.price*100:0;}
...
function bollTargetCell(s,y,type,index){const dividend=forwardGridDividend(s);if(!dividend||!s.price)return `<td class="boll-target ${type}"><div class="boll-target-box"><strong>—</strong><small>待公告分红</small></div></td>`;const target=dividend/(y/100),currentY=bollYield(s),reached=type==='buy'?currentY>=y:currentY<=y,delta=(target/s.price-1)*100,level=Math.min(index,3);return `<td class="boll-target ${type} ${reached?'reached':''} level-${level}"><div class="boll-target-box"><strong>${money(target)}</strong><small>${reached?'已达':`${delta>=0?'需涨':'需跌'} ${Math.abs(delta).toFixed(1)}%`}</small></div></td>`;}
