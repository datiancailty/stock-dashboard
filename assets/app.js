...
let personalMigrationState=null,personalLoadedParts=new Set(),personalPartLoads=new Map();
...
function renderHoldings(){
  ...
  const projectionState=noSession?'<p class="muted">请先登录，登录后从 Supabase 私有投影读取模拟盘持仓。</p>':noProjection?'<p class="muted">当前账户没有可读取的 primary 持仓投影，需由项目管理员单独授予 scope membership。</p>':rows||'<p class="muted">当前私有投影未包含持仓行；这不是公开数据推断。</p>';
  const legacyState=authSession&&personalLoadedParts.has('holdings')?`<section class="legacy-watchlist"><div class="legacy-watchlist-head"><div><b>原 Part 1 自选清单</b><small>已迁入当前账号的 Supabase 私有空间；不等于模拟盘实际持仓</small></div><span>${trackedStocks.length}只</span></div><div class="legacy-watchlist-grid">${trackedStocks.map(item=>`<span><b>${escapeHtml(item.name||item.code)}</b><small>${escapeHtml(item.code)}</small></span>`).join('')||'<p class="muted">迁移清单为空</p>'}</div></section>`:'';
  $('#holdingList').innerHTML=projectionState+legacyState;
  $('#summaryCards').innerHTML=[['模拟盘持仓',`${holdings.length}只`],['原自选清单',personalLoadedParts.has('holdings')?`${trackedStocks.length}只`:'待读取'],['最近同步',privatePortfolio?.source_generated_at?formatTime(privatePortfolio.source_generated_at):'待读取']].map(([title,value])=>`<div class="summary-card"><small>${escapeHtml(title)}</small><strong>${escapeHtml(value)}</strong></div>`).join('');
...
function renderStrategy(){
  ...
  $('#tradeRecordCount').textContent=personalLoadedParts.has('strategy')?`${count}条 · Supabase 私有迁移记录`:`待读取 · 登录后按需载入`;
...
$('#refreshStrategy').onclick=async()=>{if(!authSession)return openLogin();const button=$('#refreshStrategy');button.disabled=true;try{await refreshStrategyCloud();showMessage('私有策略记录已刷新','仅重新读取当前账号的 Part 6 私有 RPC；未调用模型、行情、VPS 或交易接口。');}finally{button.disabled=false;}};
...
function clearPersonalData(){personalMigrationState=null;personalLoadedParts=new Set();personalPartLoads=new Map();trackedStocks=[];part2Config={version:1,groups:[],extraStocks:[]};market={stocks:[],events:[],updatedAt:null};newsMemory={items:[],updatedAt:null,lastScanAt:null};tradeRecords=[];strategyFeedback=[];strategyRecommendations=[];strategyAnalysis={status:'waiting',learnedRules:[],advice:[]};strategyProfile={schemaVersion:1,externalLearnedRules:[],personalBehaviorEvidence:{},fixedGuardrails:[],nonExecutablePositionSuggestions:[],conflictsAndGaps:[]};focusedStudy=null;focusedStudyNew=null;strategyApiHealth={status:'unknown',reason:'请登录后读取私有策略记录'};}
function personalRpcForPage(pageId){return {holdings:'personal_get_part1',positions:'personal_get_part2',calendar:'personal_get_part4',news:'personal_get_part5',strategy:'personal_get_part6'}[pageId]||'';}
function applyPersonalPart(pageId,data){if(pageId==='holdings'){trackedStocks=Array.isArray(data?.watchlist)?data.watchlist.filter(item=>/^\d{6}$/.test(String(item?.code||''))).map(item=>({code:String(item.code),name:String(item.name||item.code)})):[];}else if(pageId==='positions'){part2Config=data&&typeof data==='object'?data:{version:1,groups:[],extraStocks:[]};}else if(pageId==='calendar'){market=data&&typeof data==='object'?data:{stocks:[],events:[],updatedAt:null};}else if(pageId==='news'){newsMemory=data&&typeof data==='object'?data:{items:[],updatedAt:null,lastScanAt:null};}else if(pageId==='strategy'){tradeRecords=Array.isArray(data?.trades?.records)?data.trades.records:[];strategyFeedback=Array.isArray(data?.feedback?.records)?data.feedback.records:[];strategyRecommendations=Array.isArray(data?.recommendations?.records)?data.recommendations.records:[];strategyProfile=data?.profile&&typeof data.profile==='object'?data.profile:strategyProfile;strategyAnalysis=data?.analysis&&typeof data.analysis==='object'?data.analysis:strategyAnalysis;focusedStudy=data?.focused_study&&typeof data.focused_study==='object'?data.focused_study:null;focusedStudyNew=data?.focused_study_new&&typeof data.focused_study_new==='object'?data.focused_study_new:null;strategyApiHealth=data?.api_health&&typeof data.api_health==='object'?data.api_health:{status:'unknown',reason:'私有健康记录为空'};}}
async function loadPersonalPart(pageId,force=false){const rpc=personalRpcForPage(pageId);if(!rpc||!authSession||!supabaseClient)return;if(pageId!=='holdings'&&['positions','calendar','news'].includes(pageId)&&!personalLoadedParts.has('holdings'))await loadPersonalPart('holdings');if(personalLoadedParts.has(pageId)&&!force)return;if(personalPartLoads.has(pageId))return personalPartLoads.get(pageId);const sessionAtStart=authSession;const request=(async()=>{try{const data=await supabaseRpc(rpc);if(authSession!==sessionAtStart)return;applyPersonalPart(pageId,data);personalLoadedParts.add(pageId);render();}catch(error){if(authSession===sessionAtStart)personalLoadedParts.delete(pageId);}finally{personalPartLoads.delete(pageId);}})();personalPartLoads.set(pageId,request);return request;}
function openLogin(){$('#loginError').textContent='';if(!$('#loginDialog').open)$('#loginDialog').showModal();}
function updateAuthUI(){const logged=!!authSession;const personalReady=Number(personalMigrationState?.source_files||0)>0;$('#authStatus').textContent=logged?(currentUsername?`已登录：${currentUsername}`:'已登录'):'未登录';$('#authStatus').classList.toggle('logged',logged);$('#loginButton').textContent=logged?'退出登录':'用户名/密码登录';document.body.classList.toggle('authenticated',logged);if($('#authStrip'))$('#authStrip').firstElementChild.textContent=logged?(personalReady?'当前浏览器会话已保持；Part 0—6 共用 Supabase 用户会话，私有历史按页面读取。':'当前浏览器会话已保持；Part 0—6 共用 Supabase 用户会话，私有历史等待迁入。'):'公开浏览模式：登录后才能读取私有持仓、运行投影和管理员白名单控制。';}
